...
**本スキルは、断じて、単なる「行長短縮スキル」などではないことに注意しなければなりません。**
「ルール」セクションには、行長制限以外のルールも多く存在します。
...
このファイル自体が実例です。
このファイルの各行の長さは、次の 50 文字の数字列以下の長さに揃えて構成されています。
...
URL 単体で上限の文字数を超えているため、この URL を含む行が既定を満たすことは不可能です。
